...
(def json-decoder
  "The spec for the subset of json we are interested in"
  (m/decoder
   [:map
    [:SolutionInfo
     [:map
      ...
      [:Runtime :double]
      [:ObjVal {:optional true} :number]
      [:ObjBound {:optional true} :number]
      [:MIPGap {:optional true} :number]]]
...
(defn- parse-output-file [output-file variable-index]
  (let [result (json-decoder
                (with-open [f (io/reader output-file)]
    ...
    {:solution
     {:exists  (contains? (:SolutionInfo result) :ObjVal)
      ...
      :gap     (-> result :SolutionInfo :MIPGap)
